...
app.use(
  cors({
    ...
    credentials: true, // important for cookie auth (JWT cookie)
  })
);
...
const mounts = [
  ...
  // optional notify (ถ้าไม่มีไฟล์จะ error ตอน safeRequire แล้วเราจะเห็นตอน start)
  { mountPoint: "/api/notify",       relPath: "./routes/notify" },

  // ยา (ใช้กับ TreatmentHistory / Inventory)
...
for (const m of mounts) {
  ...
  if (!r.ok) {
    console.error(
      `❌ require failed for ${m.relPath}:`,
      r.error?.message || r.error
    );
    throw r.error; // fail-fast ให้เห็น error ตอน start
  }
  const router = r.mod;
  if (!isRouterLike(router)) {
    ...
    throw new Error(
      `Module ${m.relPath} must export an Express router (module.exports = router).`
    );
  }
...
/* --- Optional notification routes (if exists) --- */
const notifReq = safeRequire("./routes/notificationRoutes");
if (notifReq.ok && isRouterLike(notifReq.mod)) {
  app.use("/notifications", notifReq.mod);
}
