...
router.get('/', async (req, res) => {
  try {
    const items = await Vaccination.find();
    res.json(items);
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  try {
    const item = await Vaccination.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(400).json({ error: err.message });
...
router.delete('/:id', async (req, res) => {
  try {
    const item = await Vaccination.findByIdAndDelete(req.params.id);
    if (!item) return res.status(404).json({ error: 'Not found' });
