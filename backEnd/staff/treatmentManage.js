...
async function resolveDoctor({ req, branchId, fallbackExistingDoctorId }) {
  ...
  const rawId =
    req.body.doctorId || req.body.staffId || fallbackExistingDoctorId;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { symptoms, diagnosis, notes, attachments, treatmentDate } =
        req.body;
...
/* DELETE  */
// DELETE /api/staff/treatments/:ownerId/:petId/:treatId
router.delete(
  "/treatments/:ownerId/:petId/:treatId",
  auth,
...
/*  LIST MEDICINES BY BRANCH  */
// GET /api/staff/medicines?branchId=... (&all=1 สำหรับ superAdmin)
router.get(
  "/medicines",
  auth,
  role(["staff", "branchAdmin", "doctor", "superAdmin"]),
  async (req, res) => {
    try {
      const wantAll =
        String(req.query.all || "") === "1" && canSeeAll(req);
      ...
      if (!wantAll) {
        ...
        const chk = assertBranch(req, bid);
        if (!chk.ok) {
