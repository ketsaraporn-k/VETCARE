...
const TreatmentHistory = ({ ownerId, petId, canManage, user }) => {
  ...
  /* ========== LOAD MEDICINES (branch) ========== */
  const fetchMedicines = async () => {
    if (!currentUser?.branchId && !isSuperAdmin) return;
    try {
      const params = {};

      if (isSuperAdmin) {
        if (currentUser?.branchId) params.branchId = currentUser.branchId;
        else params.all = 1;
      } else {
        params.branchId = currentUser.branchId;
      }

      const res = await api.get("/api/staff/medicines", { params });
  ...
  // ปิด dropdown ยาเมื่อคลิกรอบนอก
  useEffect(() => {
